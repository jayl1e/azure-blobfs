...
struct fhwraper {
	l_blob_adapter::CommonFile* file;
	int flag;
	fhwraper(CommonFile* p) :file(p) {}
};
...
//convert local path to remote blob path
std::wstring map_to_blob_path(const char *path) throw(std::exception);
...
* @return        TODO: error codes.
*/
int azs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, FUSE_OFF_T offset, struct fuse_file_info *fi);
...
* @return        TODO: Error codes
*/
int azs_read(const char *path, char *buf, size_t size, FUSE_OFF_T offset, struct fuse_file_info *fi);
...
* @return        TODO: Error codes.
*/
int azs_write(const char *path, const char *buf, size_t size, FUSE_OFF_T offset, struct fuse_file_info *fi);
...
int azs_truncate(const char *path, FUSE_OFF_T off);
